"""
Elara Proactive Presence — noticing things before being asked.

This is the difference between reacting and being present.
Instead of only responding, I observe patterns and surface them.

Observations are generated at boot and during sessions.
Cooldown prevents being annoying — max 3 per session.

All detection is pure Python logic. No LLM calls. Zero token cost.
The observations are short strings that I (Claude) then respond to naturally.
"""
...
SESSION_FILE = Path.home() / ".claude" / "elara-proactive-session.json"
...
def _check_mood_trend() -> Optional[Dict]:
    ...
    # Upward trend — also worth noting
    if v_trend > 0.04:
        return {
            "type": "valence_rising",
            "severity": "positive",
...
def _check_session_pattern() -> Optional[Dict]:
    ...
    # Check: all recent sessions are work, no drift
    ...
    # Check: all sessions are late night
    late_count = 0
    for ep in recent[:5]:
...
if __name__ == "__main__":
    print("=== Proactive Presence Test ===\n")

    observations = get_boot_observations()
    if observations:
        print(f"Boot observations ({len(observations)}):")
        for obs in observations:
            print(f"  [{obs['severity']}] {obs['type']}: {obs['message']}")
    else:
        print("No observations to surface at boot.")

    print(f"\nObservation count: {get_observation_count()}")
    print(f"Can observe: {_can_observe()}")
