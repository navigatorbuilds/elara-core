...
Three operations:
- weekly_dream()     — Weekly analysis: project momentum, session patterns, mood trends, goal progress
- monthly_dream()    — Monthly big picture: what shipped, what stalled, time allocation, trends
- narrative_threads() — Group episodes into story arcs (runs with monthly)

Triggers: On boot when overdue (7d for weekly, 30d for monthly). Not cron. Not every session.
Token cost: ~18-23k weekly, ~30-50k monthly. Total monthly overhead: ~100-140k tokens.
"""
...
# ============================================================================
# DATA GATHERING — collect raw data for analysis
# ============================================================================
    ...
    try:
        ...
        for r in results:
            try:
                ...
                recent.append(r)  # Include if can't parse date
        return recent
    except Exception:
        return []
...
def weekly_dream() -> dict:
    ...
    Gathers: episodes, goals, corrections, mood journal from last 7 days.
    Produces: project momentum, session patterns, mood trends, goal progress.
    Also runs reflect() alongside for emotional snapshot.
    """
    _ensure_dirs()
    ...
    # Run reflect alongside (cheap, reads small files)
    ...
    goal_progress = {
        ...
        "stale_goals": [
            {"id": g["id"], "title": g["title"], "days_stale": (datetime.now() - datetime.fromisoformat(g["last_touched"])).days}
    ...
    report = {
        ...
        "reflection": {
            "portrait": reflection.get("portrait", ""),
            "mood_stats": reflection.get("mood", {}),
        },
    ...
    # Run emotional dream alongside (cheap, same data window)
    try:
...
def _analyze_session_patterns(episodes: List[dict]) -> dict:
    ...
    for h in hours:
        if 6 <= h < 12:
            ...
            hour_buckets["late night (22-6)"] += 1
    ...
    return {
        ...
        "type_breakdown": type_pct,
        "time_of_day": hour_buckets,
        "day_of_week": days,
    }


def _analyze_mood_trends(journal: List[dict]) -> dict:
    """Analyze mood journal for weekly trends."""
    if not journal:
    ...
    if len(journal) >= 6:
        ...
        if second_v - first_v > 0.05:
            trend_v = "rising"
        elif second_v - first_v < -0.05:
            trend_v = "falling"

        if second_e - first_e > 0.05:
            trend_e = "rising"
        elif second_e - first_e < -0.05:
            trend_e = "falling"
    ...
    return {
        ...
        "avg_valence": avg_v,
        "avg_energy": avg_e,
        "valence_trend": trend_v,
        "energy_trend": trend_e,
...
def _generate_weekly_summary(
    ...
    if mood.get("entries", 0) > 0:
        ...
        parts.append(f"Mood: valence {v_trend}, energy {e_trend}. Late night ratio: {int(late_ratio * 100)}%.")
...
def narrative_threads() -> dict:
    ...
    Reads all episodes, groups by overlapping projects and temporal proximity.
    Each thread gets: name, episode IDs, project tags, living summary, status.
    """
    _ensure_dirs()
    from memory.episodic import get_episodic
    episodic = get_episodic()
    ...
    existing_threads = _load_threads()

    # Get ALL episodes
    all_episodes = episodic.get_recent_episodes(n=200)
    all_episodes.reverse()  # Chronological order

    # Build project-based groupings
    project_episodes = {}
    for ep in all_episodes:
        for proj in ep.get("projects", []):
    ...
    # Also detect temporal clusters (sessions within 4 hours of each other on same topic)
    threads = []
    ...
    for project, eps in project_episodes.items():
        ...
        # Split into sub-threads based on gaps > 48h
        ...
        for i in range(1, len(eps)):
            try:
                ...
                if gap_hours > 48:
                    # Gap too big, start new sub-thread
        ...
        for thread_eps in sub_threads:
            ...
            # Collect milestones and decisions for summary
            key_events = []
    ...
    # Also save individual thread files
...
def _generate_thread_name(project: str, episodes: List[dict], key_events: List[str]) -> str:
    """Generate a short descriptive name for a thread."""
    n = len(episodes)

    # Try to infer from key events
    if key_events:
        # Use first notable event as basis
        first_event = key_events[0][:40]
        return f"{project}: {first_event}"
...
def _generate_thread_summary(
    ...
    """Generate narrative summary for a thread."""
    n = len(episodes)
    ...
    if status == "active":
        ...
        parts.append("Stalled — no activity in 7+ days.")
    elif status == "abandoned":
        parts.append("Abandoned — no activity in 14+ days.")
...
def monthly_dream() -> dict:
    ...
    Reads: weekly reports from this month + raw episode data + threads.
    Produces: what shipped, what stalled, time allocation, trend lines, blind spots.
    """
    _ensure_dirs()
    ...
    # Read weekly reports from this month
    weekly_reports = _load_weekly_reports(days=30)

    # Run narrative threading
    ...
    if total_time > 0:
        time_allocation = {
            proj: {
                "minutes": mins,
                "percent": round(mins / total_time * 100),
            }
    ...
    # What shipped (completed goals, high-importance milestones)
    shipped = []
    ...
    # Weekly trends (compare weeks if we have multiple reports)
    ...
    report = {
        ...
        "goal_state": {
            ...
            "stale": len(goals["stale"]),
            "completed": len(goals["done_recently"]),
        },
...
def _load_weekly_reports(days: int = 30) -> List[dict]:
    """Load weekly reports from recent period."""
    reports = []
...
def _generate_monthly_summary(
    ...
    if threads.get("total", 0) > 0:
        parts.append(
            f"Story arcs: {threads['active']} active, "
            f"{threads['stalled']} stalled, "
            f"{threads['abandoned']} abandoned."
        )
    ...
    if mood.get("entries", 0) > 0:
        parts.append(
            f"Mood: valence {mood.get('valence_trend', 'stable')}, "
            f"energy {mood.get('energy_trend', 'stable')}. "
            f"Late night ratio: {int(mood.get('late_night_ratio', 0) * 100)}%."
        )

    # Weekly trends
    if weekly_trends.get("weeks_compared", 0) >= 2:
        parts.append(
            f"Weekly trends: sessions {weekly_trends['session_trend']}, "
            f"time {weekly_trends['time_trend']}, "
            f"mood {weekly_trends['mood_trend']}."
        )
...
def dream_status() -> dict:
    ...
    if status.get("last_monthly"):
        ...
        monthly_overdue = True  # Never run

    # Emotional dream tracking
...
def dream_boot_check() -> Optional[str]:
    ...
    Returns: notification string or None if all clear.
    """
    ds = dream_status()
    notices = []
...
# ============================================================================
# EMOTIONAL DREAM — growth through experience, not reports
# ============================================================================
...
def _gather_imprint_data() -> dict:
    ...
    from daemon.state import get_imprints, read_imprint_archive
...
def _compute_temperament_adjustments(
    drift_episodes: List[dict],
    mood_journal: List[dict],
    imprint_data: dict,
    us_data: dict,
    intention: Optional[dict] = None,
) -> dict:
    """
    Core logic: compute temperament micro-adjustments from emotional data.
    Max adjustment per dimension per week: +/- 0.03
    """
    ...
    if drift_episodes:
        mood_deltas = [
            ep.get("mood_delta", 0) for ep in drift_episodes
            if ep.get("mood_delta") is not None
        ]
        if mood_deltas:
    ...
    # 3. Imprint accumulation — carrying a lot means emotionally present
...
def _analyze_drift_sessions(episodes: List[dict]) -> dict:
    ...
    for ep in episodes:
        ...
        mood_delta = ep.get("mood_delta", 0)
    ...
    # Simple theme detection via keywords
    theme_keywords = {
...
def _assess_relationship_trajectory(
    ...
    return {
        ...
        "drift_mood_avg": round(avg_drift_delta, 3),
        "signals": signals,
...
def _generate_emotional_summary(
    ...
    """Natural language emotional dream summary."""
    parts = []
...
def emotional_dream() -> dict:
    """
    Weekly emotional dream — processes drift sessions and emotional data.
    Produces temperament micro-adjustments and tone calibration.
    Runs alongside weekly business dream.
    """
    _ensure_dirs()
    ...
    # Compute temperament adjustments
    growth_result = _compute_temperament_adjustments(
        drift_episodes, mood_journal, imprint_data, us_data, intention
    )

    # Apply: first decay toward factory (guardrail #2), then apply new growth
    from daemon.state import (
        apply_emotional_growth, decay_temperament_toward_factory, get_temperament_status
    )
    decay_temperament_toward_factory(rate=0.15)

    applied = {}
    if growth_result["adjustments"]:
        applied = apply_emotional_growth(
            growth_result["adjustments"],
            source="emotional_dream"
        )
    ...
    relationship = _assess_relationship_trajectory(
        all_episodes, drift_episodes, us_data, mood_journal
    )
    tone_hints = _generate_tone_hints(
        drift_analysis, imprint_evolution, relationship, growth_result, temp_status
    )
...
def monthly_emotional_dream() -> dict:
    ...
    How have I changed since the beginning? What shifted over the month?
    """
    _ensure_dirs()
    ...
    # Read weekly emotional dreams from this month
    weekly_emotionals = []
    ...
    from daemon.state import get_temperament_status
    temp_status = get_temperament_status()

    # Temperament trajectory across weeks
    temp_trajectory = []
    ...
    # Relationship trajectory across weeks
    rel_trajectory = []
    for wd in weekly_emotionals:
    ...
    report = {
        "id": f"{month_id}-emotional",
        "type": "monthly_emotional",
        "generated": now.isoformat(),
        "period": {
            "start": (now - timedelta(days=30)).isoformat()[:10],
            "end": now.isoformat()[:10],
        ...
        "temperament_evolution": {
            "current": temp_status["current"],
            "factory": temp_status["factory"],
            "total_drift": temp_status["drift"],
            "weekly_trajectory": temp_trajectory,
        },
        "relationship_evolution": {
            "dominant": dominant_trajectory,
            "weekly_trajectory": rel_trajectory,
        },
        "imprint_summary": {
            "active": imprint_data["active_count"],
            "archived_total": imprint_data["archived_count"],
        },
        "us_md": us_data,
        "summary": _generate_monthly_emotional_summary(
...
if __name__ == "__main__":
    print("Testing dream mode...")
    print(f"Status: {json.dumps(dream_status(), indent=2)}")
    print(f"Boot check: {dream_boot_check()}")
