...
# Factory defaults — the unchanging original. Who I was at genesis.
FACTORY_TEMPERAMENT = {
    "valence": 0.55,
    "energy": 0.5,
    "openness": 0.65,
...
# Maximum drift from factory values (guardrail #1)
TEMPERAMENT_MAX_DRIFT = 0.15

# Temperament adjustment log (guardrail #3: transparency)
...
# ============================================================================
# EMOTIONAL GROWTH SYSTEM — temperament shaped by experience
# ============================================================================

def _clamp_temperament(temperament: dict) -> dict:
    """Enforce bounds: temperament can't drift more than TEMPERAMENT_MAX_DRIFT from factory."""
    for key in ["valence", "energy", "openness"]:
...
def apply_emotional_growth(adjustments: dict, source: str = "emotional_dream") -> dict:
    ...
    Args:
        adjustments: dict of {dimension: delta} e.g. {"valence": 0.02, "openness": -0.01}
        source: what triggered this (for logging)

    Returns:
        Updated temperament with adjustment details
    """
    state = _load_state()
...
def decay_temperament_toward_factory(rate: float = 0.15) -> dict:
    """
    Natural decay of temperament adjustments toward factory baseline.
    Call weekly. Rate 0.15 means 15% of the drift decays per call.
    ~2 weeks without reinforcement returns to near-factory (guardrail #2).
    """
    state = _load_state()
    temperament = state.get("temperament", TEMPERAMENT.copy())
...
def reset_temperament() -> dict:
    """Nuclear option (guardrail #6): reset temperament to factory defaults."""
    state = _load_state()
...
if __name__ == "__main__":
    ...
    print(f"Temperament status: {json.dumps(get_temperament_status(), indent=2)}")
